...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers.filter(programmer => programmer.id === selected)[0].name;

  };
  const textColor = selected ? 'gold' : 'royalblue';
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setSelected(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
